...
export default function PieChartComponent() {
    const { isLoading, data: x } = useGetAllCathedralXQuery<any>({}, { refetchOnMountOrArgChange: true });
    const { data } = useGetSeedAnalyticsQuery({});

    const [monthlyData, setMonthlyData] = useState([]);

    useEffect(() => {
        if (data?.seed?.last12Months) {
            const sortedData: any = [...data.seed.last12Months]?.sort((a, b) => new Date(b.month).getTime() - new Date(a.month).getTime());
            setMonthlyData(sortedData);
        }
    }, [x]);

    const aggregateData = (data: any) => {
        return x?.seed?.reduce((acc: any, offering: any) => {
            ...
            <div className='flex flex-col gap-5'>
                ...
                <div className=''>
                    <div className='flex justify-between'>
                        <h4 className='text-blue-500 font-medium text-lg mb-4'>Monthly Total Sacrifice Income</h4>
                        <button
                            onClick={downloadCSV}
